...
| `se-energimyndigheten` | Two XLSX workbooks            | Swedish CPO and EMSP identifiers                                      | The connector combines separate CPO and EMSP workbooks. `machineReadableUrl` points only to the primary CPO workbook; each record retains its actual workbook URL. Both are fetched under `/globalassets/`, without the cache-busting segment the landing page rewrites on every upload. |
