...
redistribute. Spain uses a statutory reuse basis, while Via Lietuva publishes an
explicit licence.
...
| `fi-traficom`          | HTML table                    | Finnish identifiers with explicit CPO or MSP roles                    | The page contains other observed IDs, but the connector skips them when their roles are not explicit.                                                              |
