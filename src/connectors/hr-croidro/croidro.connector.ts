...
const SOURCE_HOSTS = ["pametnamobilnost.hr"];

export class CroIdroConnector implements RegistryConnector<CroIdroCsvRow> {
  readonly sourceId = "hr-croidro";
  ...
  async parse(input: ParseInput): Promise<ParseOutput<CroIdroCsvRow>> {
    try {
      return parseCroIdroCsv(input.body);
    } catch (error) {
      return {
        records: [],
        warnings: [],
  ...
  async normalize(input: NormalizeInput<CroIdroCsvRow>): Promise<NormalizeOutput> {
    ...
    for (const sourceRecord of input.records) {
      const organizationName =
        clean(sourceRecord.companyName) ?? "Unspecified Croatian IDRO organization";
      const identifiers = [
      ...
      for (const identifier of identifiers) {
        ...
        if (seen.has(key)) {
          warnings.push({
            severity: "warning",
            sourceId: input.source.id,
            code: "CROIDRO_DUPLICATE_IDENTIFIER",
            message: `Duplicate Croatian IDRO identifier ${identifier.sourceValue} for ${identifier.role}.`,
          });
          continue;
        }
        seen.add(key);
        ...
        records.push({
          ...
          metadata: {
            croIdroCompanyName: organizationName,
            croIdroSeparator: parsed.separator,
          },
        });
      }
    }
...
function parseCroIdroIdentifier(value: string) {
  const match = /^(HR)([*-])([A-Za-z0-9]{3})$/i.exec(value.trim());
